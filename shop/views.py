...
def order_product(request, product_id):
    ...
    if form.is_valid():
        try:
            cart_product = Order.objects.get(user_fk = request.user.id, 
                            accepted = False, product_fk = product.id)
        except :
            cart_product = ''
        if cart_product:
...
def cart_read(request):
    ...
    sum_cost = Order.objects.filter(user_fk = request.user.id,
     accepted = False).aggregate(Sum('cost'))
    return render(request, 'shop/cart.html', 
    context={'cart':cart, 'sum_cost':sum_cost})
...
def ordered_list(request):
    ordered = Order.objects.filter(user_fk = request.user.id,
     accepted = True).order_by('-date_order')[:5]#.distinct('order_code')[:5]
...
def read_ordered(request, order_code):
    ...
    return render(request, 'shop/cart.html', context={'cart':ordered,
                                                 'sum_cost':sum_cost})
...
class Product_Delete(PermissionRequiredMixin, View):
